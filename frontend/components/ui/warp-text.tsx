...
// ============================================================
// WarpText — React Bits (TypeScript port)
// WebGL-based glass-warp text with pointer interaction
// Dependency: ogl (already installed)
// ============================================================
...
void main() {
  ...
  float bulge = (dist / radius) * (1.0 - dist / radius) * (1.0 - dist / radius) * 6.75 * uPointerActive;
...
const measureLine = (ctx: CanvasRenderingContext2D, line: string, letterSpacing: number): number => {
  ...
  return chars.reduce((w, c) => w + ctx.measureText(c).width, 0) +
...
const drawLine = (
  ...
  chars.forEach((char, i) => {
    ...
    cursor += ctx.measureText(char).width + (i === chars.length - 1 ? 0 : letterSpacing);
  });
};
...
interface BuildTextProps {
  ...
  height: number;
  dpr: number;
  props: Required<Pick<WarpTextProps, 'text' | 'color' | 'fontSize' | 'fontWeight' | 'fontFamily' | 'letterSpacing' | 'lineHeight'>>;
}

const buildTextCanvas = ({ container, width, height, dpr, props }: BuildTextProps): HTMLCanvasElement => {
...
export default function WarpText({
  ...
  const propsRef = useRef({ text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple });
  const contextRef = useRef<{ program: any; rasterize: () => void } | null>(null);

  // Sync prop changes to the live WebGL context
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let renderer: any, gl: any, program: any, geometry: any, mesh: any, texture: any;
    let resizeObserver: ResizeObserver, intersectionObserver: IntersectionObserver;
    let raf = 0, disposed = false, contextLost = false;
    ...
    const rasterize = async () => {
      ...
      texture.image = buildTextCanvas({ container, width: rect.width, height: rect.height, dpr, props: propsRef.current as any });
  ...
  return (
    <div
      ref={containerRef}
      className={`warp-text-container ${className}`.trim()}
      style={{ position: 'relative', display: 'block', width: '100%', overflow: 'hidden', isolation: 'isolate', borderRadius: 'inherit', ...style }}
      role="img"
      aria-label={text}
    />
  );
